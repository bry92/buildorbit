 ...
 *
 * KNOWN LIMITATION: All stores are in-memory.
 *   1. State resets on every Render restart (each deploy triggers a restart).
 *   2. If the app runs on multiple instances, each has independent counters.
 *      An attacker could exceed per-IP limits by routing across instances.
 *
 * This is acceptable for the current single-instance deployment on Render.
 * Upgrade path if needed: Redis-backed counters with atomic INCR + EXPIRE
 * (Upstash Redis is the lowest-friction option for Render).
 */
