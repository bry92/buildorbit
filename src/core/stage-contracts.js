...
function buildStageInput(stage, prompt, previousOutputs = {}) {
  ...
  switch (stage) {
    ...
    case 'code':
      ...
      //
      // EXCEPTION: extend_existing mode skips required_files checks — the existing repo
      // architecture is authoritative; we extend it rather than replacing it.
      ...
      const _isExtendExisting = previousOutputs.scaffold && previousOutputs.scaffold._repo_mode === 'extend_existing';
      if (intentClass && !_isExtendExisting) {
...
function validateScaffoldManifest(scaffold, intentClass) {
  ...
  //    EXCEPTION: extend_existing mode uses the ACTUAL repo files as the manifest.
  //    Required-file checks are skipped because the existing repo architecture is
  //    authoritative — we are extending it, not replacing it.
  const isExtendExisting = scaffold._repo_mode === 'extend_existing';
  if (intentClass && Array.isArray(scaffold.files) && !isExtendExisting) {
    const schema = getScaffoldSchema(intentClass);
