...
class PipelineEventBus extends EventEmitter {
   ...
   *
   * Wrapped with try-catch: synchronous listener throws become unhandled
   * rejections if not caught here. Logging failures must never crash the server.
   */
  emitBusEvent(eventName, payload) {
    ...
    try {
      ...
      // A listener threw synchronously. Log and continue — never crash the pipeline.
      console.error(`[EventBus] Listener error on "${eventName}" (non-fatal):`, listenerErr.message);
    }
